...
def main():
    ...
    target_modules = {k: named_modules[k] for k in named_modules if 
               isinstance(named_modules[k], Conv2d)
               or isinstance(named_modules[k], Linear)
               or isinstance(named_modules[k], Embedding)
    }
    unet_lora_config = LoraConfig( r=4, lora_alpha=4, init_lora_weights="gaussian", target_modules=target_modules)
